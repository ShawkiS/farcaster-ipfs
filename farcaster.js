...
const getNode = () => {
  return new providers.AlchemyProvider('rinkeby', "A0vKzDWq-CkZa6k9dQdne2a4BjnwGq_-");
}
...
const verifyCast = (body, hash) => {
    const calculatedHash = utils.keccak256(utils.toUtf8Bytes(JSON.stringify(body)));
...
exports.getAllCasts = async () => {
  ...
  const directoryResponse = await axios.get(directoryUrl).then((result) => { return result; }).catch((e) => console.log(e));

  const casts = await (await     
   axios.get(directoryResponse.data.body.addressActivityUrl)).data;
  return casts.filter((c) => (verifyCast(c.body, c.merkleRoot)));
}
...
exports.getPreviewCasts = async () => {
  const casts = await this.getAllCasts();
  const deletedCasts = [];
...
exports.isThereNewCasts = async (db) => {
  const casts = await this.getAllCasts();
